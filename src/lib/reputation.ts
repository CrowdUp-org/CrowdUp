...
export const REPUTATION_LEVELS = {
    ...
    contributor: { name: 'Contributor', icon: '🌿', minScore: 100, maxScore: 499 },
    established: { name: 'Established', icon: '🌳', minScore: 500, maxScore: 999 },
...
export type ReputationActionType = keyof typeof POINT_VALUES | 'manual_adjustment';
...
export async function awardPoints(
    ...
    try {
        const points = options?.customPoints ?? POINT_VALUES[actionType as keyof typeof POINT_VALUES] ?? 0;
    ...
    try {
      ...
      `)
        ...
        return data.map(item => ({
            ...item,
            post_title: item.related_post_id ? postTitles[item.related_post_id] : undefined,
