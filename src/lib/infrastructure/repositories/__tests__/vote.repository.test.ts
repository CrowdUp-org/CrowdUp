...
describe('voteRepository', () => {
  ...
  describe('create', () => {
    it('should create vote and return mapped entity', async () => {
      ...
      const result = await voteRepository.create(mockCreateDTO, mockVoteRow.user_id);
  ...
  describe('findByPostAndUser', () => {
    it('should return vote when found', async () => {
      const mockChain = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        maybeSingle: vi.fn().mockResolvedValue({ data: mockVoteRow, error: null }),
      };
      (supabase.from as Mock).mockReturnValue(mockChain);
    ...
    it('should return null when no vote exists', async () => {
      ...
      const result = await voteRepository.findByPostAndUser('post-id', 'user-id');
  ...
  describe('findByPost', () => {
    it('should return all votes for a post', async () => {
      const mockVotes = [
        mockVoteRow,
        { ...mockVoteRow, id: 'vote-2', user_id: 'user-2', vote_type: 'down' as const },
      ];
  ...
  describe('findByUser', () => {
    it('should return votes by user with pagination', async () => {
      ...
      const result = await voteRepository.findByUser(mockVoteRow.user_id, 50, 0);
  ...
  describe('update', () => {
    ...
    it('should throw error when update fails', async () => {
      ...
      await expect(voteRepository.update(mockVoteRow.id, 'down')).rejects.toThrow(
        'Failed to update vote: Update failed'
      );
    });
  });
  ...
  describe('delete', () => {
    it('should delete vote successfully', async () => {
      ...
      await expect(voteRepository.delete(mockVoteRow.id)).resolves.toBeUndefined();
  ...
  describe('deleteByPostAndUser', () => {
    it('should delete vote by post and user', async () => {
      ...
      finalMockChain.eq.mockReturnValueOnce(finalMockChain).mockResolvedValueOnce({ error: null });

      (supabase.from as Mock).mockReturnValue(finalMockChain);

      await expect(
        voteRepository.deleteByPostAndUser(mockVoteRow.post_id, mockVoteRow.user_id)
