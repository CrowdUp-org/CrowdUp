...
export async function requestVerification(
    ...
    try {
        ...
        if (memberError || !membership) {
            return { success: false, error: "You must be a member of this company to request verification" };
        }
        ...
        if ((membership as any).verification_status === "rejected" && (membership as any).verification_rejected_at) {
            ...
            if (daysSince < 7) {
                ...
                return { success: false, error: `Please wait ${daysRemaining} more day(s) before resubmitting` };
            }
        }
...
export async function getVerificationStatus(
    ...
    return {
        ...
        status: data.verification_status as "pending" | "approved" | "rejected" | null,
...
export async function getPendingRequests(): Promise<VerificationRequest[]> {
    ...
    return (data || []).map((item: any) => ({
        ...item,
        verification_documents: item.verification_documents as VerificationDocuments | null,
        user: item.users,
