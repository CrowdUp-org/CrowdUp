...
export async function approveVerification(adminUserId: string, memberId: string) {
    const isAdmin = await checkIsAdmin(adminUserId);
    if (!isAdmin) {
        throw new Error("Unauthorized");
    }
...
export async function rejectVerification(adminUserId: string, memberId: string, reason: string) {
    const isAdmin = await checkIsAdmin(adminUserId);
    if (!isAdmin) {
        throw new Error("Unauthorized");
    }
