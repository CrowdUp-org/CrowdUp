...
describe("voteService", () => {
  ...
  describe("toggleVote", () => {
    describe("creating a new vote", () => {
      it("should create new vote when no existing vote", async () => {
        ...
        vi.mocked(voteRepository.countByPost).mockResolvedValue({
          upvotes: 1,
          downvotes: 0,
        });
        vi.mocked(postRepository.updateVoteCount).mockResolvedValue({
          ...mockPost,
          votes: 1,
        });
    ...
    describe("toggling off an existing vote (same type)", () => {
      it("should remove vote when clicking same vote type", async () => {
        ...
        vi.mocked(voteRepository.findByPostAndUser).mockResolvedValue(
          existingVote,
        );
      ...
      it("should throw ValidationError on invalid input - invalid voteType", async () => {
        await expect(
          voteService.toggleVote(
            { postId: "post-1", voteType: "invalid" },
            "user-1",
          ),
        ).rejects.toThrow(ValidationError);
      });
  ...
  describe("getVoteStatusesForPosts", () => {
    it("should return vote statuses for multiple posts", async () => {
      ...
      vi.mocked(voteRepository.getVoteStatusesForPosts).mockResolvedValue(
        voteMap,
      );
  ...
  describe("getVoteCounts", () => {
    ...
    it("should throw NotFoundError when post does not exist", async () => {
      ...
      await expect(voteService.getVoteCounts("non-existent")).rejects.toThrow(
        NotFoundError,
      );
    });
  });
